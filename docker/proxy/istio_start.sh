...
if [ ${EXEC_USER} == "root" ] ; then
  /opt/istio-proxy/decode.sh
else
  sudo /opt/istio-proxy/decode.sh
fi

# Match pilot/docker/Dockerfile.proxyv2
export ISTIO_META_ISTIO_VERSION="1.13.4"
...
if [ ${EXEC_USER} == "istio-proxy" ] ; then
  ...
  exec su -s /bin/bash -c "INSTANCE_IP=${ISTIO_SVC_IP} POD_NAME=${POD_NAME} POD_NAMESPACE=${NS} exec ${ISTIO_BIN_BASE}/pilot-agent proxy ${ISTIO_AGENT_FLAGS_ARRAY[*]}" ${EXEC_USER}
fi