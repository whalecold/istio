...
func (m *mcpDiscovery) DeRegister(ctx context.Context, id string) error {
	cm, err := m.kubeClient.CoreV1().ConfigMaps(constants.IstioSystemNamespace).Get(ctx, configmapName, metav1.GetOptions{})
	...
	delete(cm.Data, id)
	_, err = m.kubeClient.CoreV1().ConfigMaps(constants.IstioSystemNamespace).Update(ctx, cm, metav1.UpdateOptions{})
	return err
}

// Register the mcp server adrress to kubernetes.
func (m *mcpDiscovery) Register(ctx context.Context, address McpServer) error {
	cm, err := m.kubeClient.CoreV1().ConfigMaps(constants.IstioSystemNamespace).Get(ctx, configmapName, metav1.GetOptions{})
	if err != nil && !errors.IsNotFound(err) {
		return err
	}
	if errors.IsNotFound(err) {
		...
		_, err = m.kubeClient.CoreV1().ConfigMaps(constants.IstioSystemNamespace).Create(ctx, cm, metav1.CreateOptions{})
		return err
	}
	// clone from memory.
	cm = cm.DeepCopy()
...
type Registry interface {
	Register(context.Context, McpServer) error
...
// New ...
func New(cli kubernetes.Interface, opts *Options) DiscoveryRegistry {
	md := &mcpDiscovery{
		kubeClient:         cli,
		configMapName:      configmapName,
		configMapNamespace: constants.IstioSystemNamespace,
