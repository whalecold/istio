...
func TestGetLocality(t *testing.T) {
	...
	ctr := &Controller{
		localityGetters: []LocalityGetter{newFakeLocalityGetter(map[string]string{
			"c1/1.1.1.1": "region1/zone1/subzone1",
			"c1/1.1.1.2": "region1/zone2/subzone1",
			"c3/1.1.1.1": "region2/zone1/subzone1",
		}), newFakeLocalityGetter(map[string]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ctr.getLocality(tt.addr, tt.wle); got != tt.expected {
