...
func TestMergeSidecarByResources(t *testing.T) {
	testCases := []struct {
		in    *model.SidecarScope
		names []string
		out   *config.Config
		desc  string
	}{
		...
		{
			desc: "empry resource name",
			in: &model.SidecarScope{
				Name:             "test1",
				Namespace:        "ns",
				SidecarNamespace: "sideNs",
			...
			in: &model.SidecarScope{
				...
				Sidecar: &networking.Sidecar{
					...
					Egress: []*networking.IstioEgressListener{
						...
						{
							// should remain none http protocol.
							Port: &networking.Port{
								Number:     3306,
								Protocol:   "TCP",
								Name:       "tcp",
						...
						},
						{
							...
							Bind:        "1.1.1.1",
							CaptureMode: networking.CaptureMode_IPTABLES,
							Hosts:       []string{"bookinfo/*", "*/hello"},
						},
					},
				},
			},
			names: []string{"8080/productpage:8080", "8080/productpage.default.svc.cluster.local:8080", "8081/productpage:8081", "9080/test.bookinfo:9080", "9080/hello.default:9080"},
			out: &config.Config{
				Meta: config.Meta{
					Name:      "test1",
					Namespace: "sideNs",
					...
					Egress: []*networking.IstioEgressListener{
						{
							Bind:        "1.1.1.1",
							CaptureMode: networking.CaptureMode_IPTABLES,
							Hosts:       []string{"*/productpage", "default/productpage", "bookinfo/test", "default/hello"},
						},
						{
							Port: &networking.Port{
								Number:     3306,
						...
						},
						{
							...
							Bind:        "1.1.1.1",
							CaptureMode: networking.CaptureMode_IPTABLES,
							Hosts:       []string{"*/productpage", "default/productpage"},
						},
						{
							Port: &networking.Port{
								Number:     8081,
						...
						},
						{
							...
							Bind:        "1.1.1.1",
							CaptureMode: networking.CaptureMode_IPTABLES,
							Hosts:       []string{"bookinfo/test", "default/hello"},
						},
					},
				},
			},
					...
					Egress: []*networking.IstioEgressListener{
						{
							...
							Bind:        "1.1.1.1",
							CaptureMode: networking.CaptureMode_IPTABLES,
							Hosts:       []string{"*/*exmple.com"},
						},
					},
				},
			},
			names: []string{"9080/www.exmple.com:9080"},
			out: &config.Config{
				Meta: config.Meta{
					Name:      "test1",
					Namespace: "sideNs",
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			got := model.MergeSidecarByResources(tc.in, tc.names)
